...
SET SUMMARY="Cronus json serializer"
SET DESCRIPTION="Cronus json serializer"


%FAKE% %NYX% "target=RunTests" appName=Elders.Cronus.Serialization.NewtonsoftJson.Tests appSummary=%SUMMARY% appDescription=%DESCRIPTION% appType=tests
if errorlevel 1 (
   echo Tests faild with exit code %errorlevel%
   exit /b %errorlevel%
)

%FAKE% %NYX% appName=Elders.Cronus.Serialization.NewtonsoftJson appReleaseNotes=%RELEASE_NOTES% appSummary=%SUMMARY% appDescription=%DESCRIPTION% nugetkey=%RELEASE_NUGETKEY% nugetPackageName=Cronus.Serialization.NewtonsoftJson
